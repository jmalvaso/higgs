...
#from higgs_cp.util import IF_NANO_V9, IF_NANO_V11
...
def study_muon_selection(
    ...
    **kwargs,
    ...
    for cut_name in selections.keys():
        buffer_mask = buffer_mask & selections[cut_name]
        selection_idx = sorted_idx[buffer_mask[sorted_idx]]
        selection_idx = ak.values_astype(selection_idx, np.int32)
...
def muon_selection(
    ...
    **kwargs,
    ...
    selection_idx = sorted_idx[selection_mask[sorted_idx]]
    selection_idx = ak.values_astype(selection_idx, np.int32)
    ...
    # )
    # # convert to sorted indices
    # veto_idx = sorted_idx[veto_mask[sorted_idx]]
    # veto_idx = ak.values_astype(veto_idx, np.int32)

    return selection_idx, veto_idx
    
# @selector(
#     uses={
#         f"Tau.{var}" for var in [
#         "pt", "eta", "dz", "idDeepTau2018v2p5VSjet", "idDeepTau2018v2p5VSe", "idDeepTau2018v2p5VSmu", "idDecayModeNewDMs"
#         ] 
...
# @selector(
#     uses={
#         muon_selection, tau_selection,
#         # nano columns
#         "event", "Muon.charge", "Tau.charge", "Electron.mass", "Muon.mass",
#         "Tau.mass",
#     },
#     produces={
#         muon_selection, tau_selection,
...
#     is_single_muon = (ak.num(muon_indices, axis=1) == 1)


#     where = (channel_id == 0) & is_single_muon
#     print(f"total number of single muons: {ak.sum(is_single_muon)}")
...
#     single_muon_events = ak.num(sel_muon_indices >= 0, axis=1)
#     single_muon_events = ak.values_astype(single_muon_events, np.bool_)
...
#     return events, SelectionResult(
#         steps={
#             "single_muon": single_muon_events  ,
...
#     # pt sorted indices for converting masks to indices
#     sorted_idx = ak.argsort(events.Tau.pt, axis=-1, ascending=False)
    
#     min_pt = 20
#     selection_mask = (
#         (events.Tau.idDeepTau2018v2p5VSjet == 5) & #Medium DeepTau VS jet working point
#         (events.Tau.idDeepTau2018v2p5VSe   == 2) & #VVLoose DeepTau VS electron working point
#         (events.Tau.idDeepTau2018v2p5VSmu == 4) & #Tight  DeepTau VS muon working point
